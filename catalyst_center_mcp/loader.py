"""
loader.py — loads Cisco Catalyst Center OpenAPI sub-specs for a given version,
...
Splitting algorithm (see issue #13 in the sdwan repo for the worked example):
...
def _derive_action_name(method: str, path: str, tag: str) -> str:
    ...
    Independent of Cisco's `operationId`, which Cisco renames between releases.
    """
    verb = method.lower()
...
_OFFSET_SIZE_PARAMS = {"limit", "count", "pageSize"}
_OFFSET_INDEX_PARAMS = {"offset", "startIndex"}


def _detect_pagination_style(
    parameters: list[ParameterSpec],
) -> Literal["scroll", "cursor", "offset"] | None:
    """Decide pagination style from a parsed parameter list.

    - "scroll" if any param is named scrollId (preserved for compatibility;
      does not appear in real Catalyst Center specs).
    - "cursor" if any param is named cursor (3.1.3 pattern, ~2 endpoints).
    - "offset" if any of offset/startIndex is present alongside a size param.
    - None otherwise.
    """
...
def _split_by_path(
    ...
    # "discriminator" that differentiates each bucket.
    ...
    def __init__(
        ...
        if not self.version_dir.exists():
            raise FileNotFoundError(
                ...
                f"Download the OpenAPI specs from Cisco DevNet and place them in "
                f"{self.version_dir}/"
            )
