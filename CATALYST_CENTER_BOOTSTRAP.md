...
- **`version` field meaning.** The `version` string inside the `{response, version}` envelope is the *API schema version* (currently `"1.0"`), not the Catalyst Center platform version. Probe `/dna/intent/api/v1/dna-system/api-versions` or a `/system/v1/*` endpoint to discover the platform version — confirmed against the always-on sandbox in 2026-05-25 auth findings.
